...
# User solution path (optional: pass path as first arg)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)"
USER_DIR="${1:-../../../../rendu/life}"

# If the provided/default user dir doesn't exist but the script directory
# contains solution files, use the script directory (local testing).
if [ ! -d "$USER_DIR" ]; then
    CANDIDATES=$(find "$SCRIPT_DIR" -maxdepth 1 -type f -name "*.c" -o -name "*.h" | wc -l)
...
# Compile user (capture output and show on failure)
gcc -Wall -Wextra -Werror -std=c99 -o user_life carlos-j_life.c >user_compile.log 2>&1
if [ $? -ne 0 ]; then
    echo -e "${RED}❌ User compilation failed! See below for errors:${NC}"
    sed -n '1,200p' user_compile.log
    cd - >/dev/null
    rm -rf "$TMP_DIR"
    exit 1
fi
